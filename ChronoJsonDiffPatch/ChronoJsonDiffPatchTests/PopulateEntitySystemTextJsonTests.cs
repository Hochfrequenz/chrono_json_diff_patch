...
/// <summary>
/// Twin tests for <see cref="PopulateEntityTests"/> that use System.Text.Json instead of Newtonsoft.Json
/// for the <c>populateEntity</c> action.
///
/// Since System.Text.Json doesn't have a direct equivalent to <c>JsonConvert.PopulateObject</c>,
/// we use the .NET 8+ <see cref="JsonObjectCreationHandling.Populate"/> feature with a custom
/// <see cref="JsonTypeInfo"/> modifier to inject the target instance.
/// </summary>
public class PopulateEntitySystemTextJsonTests
{
    ...
    /// <summary>
    /// Creates a populateEntity action for a specific type using System.Text.Json.
    /// This uses the .NET 8+ JsonTypeInfo modifier to inject an existing instance.
    /// </summary>
    private static Action<string, T> CreateSystemTextJsonPopulateAction<T>()
        where T : class
    {
        return (json, target) =>
        {
    ...
    /// <summary>
    /// Creates a chain with System.Text.Json-based populateEntity configured for in-place updates
    /// </summary>
    private static TimeRangePatchChain<T> CreateChainWithPopulate<T>()
        where T : class
    {
        return new TimeRangePatchChain<T>(populateEntity: CreateSystemTextJsonPopulateAction<T>());
    }

    /// <summary>
    /// Creates a chain with System.Text.Json-based populateEntity and existing patches
    /// </summary>
    private static TimeRangePatchChain<T> CreateChainWithPopulate<T>(
        ...
        where T : class
    {
        return new TimeRangePatchChain<T>(
            timeperiods,
            patchingDirection,
            populateEntity: CreateSystemTextJsonPopulateAction<T>()
        );
    }
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Single_Patch"/> using System.Text.Json
    /// </summary>
    [Theory]
    [InlineData(-1, "foo")]
    [InlineData(0, "bar")]
    [InlineData(1, "bar")]
    public void Test_Single_Patch_WithSystemTextJson(int daysToKeyDate, string expectedProperty)
    {
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Three_Patches_Sequentially"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Three_Patches_Sequentially_WithSystemTextJson()
    {
        var trpCollection = CreateChainWithPopulate<DummyClass>();
        var myEntity = new DummyClass { MyProperty = "foo" };
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Three_Patches_Sequentially_with_the_last_one_being_at_the_same_date"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Three_Patches_Sequentially_SameDate_WithSystemTextJson()
    {
        var trpCollection = CreateChainWithPopulate<DummyClass>();
        var myEntity = new DummyClass { MyProperty = "foo" };
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Three_Patches_Sequentially_with_middle_last_one_being_at_the_same_date_keep_the_future"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Three_Patches_KeepTheFuture_WithSystemTextJson()
    {
        var trpCollection = CreateChainWithPopulate<DummyClass>();
        var myEntity = new DummyClass { MyProperty = "foo" };
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Three_Patches_Sequentially_with_middle_last_one_being_at_the_same_date_overwrite_the_future"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Three_Patches_OverwriteTheFuture_WithSystemTextJson()
    {
        var trpCollection = CreateChainWithPopulate<DummyClass>();
        var myEntity = new DummyClass { MyProperty = "foo" };
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Three_Patches_Unordered"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Three_Patches_Unordered_WithSystemTextJson()
    {
        var trpCollection = CreateChainWithPopulate<DummyClass>();
        var myEntity = new DummyClass { MyProperty = "foo" };
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Four_Patches_Unordered"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Four_Patches_Unordered_WithSystemTextJson()
    {
        var trpCollection = CreateChainWithPopulate<DummyClass>();
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Patching_Many_Dates"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Patching_Many_Dates_WithSystemTextJson()
    {
    ...
    /// <summary>
    /// Twin of <see cref="TimeRangePatchChainTests.Test_Patching_On_Same_Date"/> using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Patching_On_Same_Date_WithSystemTextJson()
    {
    ...
    /// <summary>
    /// Twin of <see cref="ShowCaseTests.Test_Three_Patches_Sequentially"/> (the bicycle example) using System.Text.Json
    /// </summary>
    [Fact]
    public void Test_Bicycle_ShowCase_WithSystemTextJson()
    {
    ...
    /// <summary>
    /// Test reverse chain with System.Text.Json (AntiParallelWithTime)
    /// </summary>
    [Fact]
    public void Test_Reverse_Chain_WithSystemTextJson()
    {
        ...
        var antiParallelChain = CreateChainWithPopulate<Bicycle>(
    ...
    /// <summary>
    /// Demonstrates that System.Text.Json populateEntity produces same results as regular PatchToDate
    /// </summary>
    [Fact]
    public void Test_SystemTextJson_Produces_Same_Results_As_Regular_PatchToDate()
    {
        // Create two identical chains - one with System.Text.Json populateEntity, one without
    ...
    #region Nested Entity Tests - Demonstrating System.Text.Json Behavior
    ...
    /// <summary>
    /// GOOD NEWS: System.Text.Json with JsonObjectCreationHandling.Populate PRESERVES nested object references!
    ...
    /// </summary>
    [Fact]
    public void Test_PopulateEntity_Preserves_Nested_Object_References_With_SystemTextJson()
    {
        var chain = CreateChainWithPopulate<BicycleWithOwner>();
        ...
        // GOOD: System.Text.Json with Populate mode preserves the Owner reference!
        ReferenceEquals(targetBicycle.Owner, originalOwnerRef)
            .Should()
            .BeTrue(
                "System.Text.Json with Populate preserves nested object references - ORM tracking is safe!"
            );
    ...
    /// <summary>
    /// GOOD NEWS: System.Text.Json with JsonObjectCreationHandling.Populate preserves collection references!
    /// Note: Like Newtonsoft.Json, it APPENDS to collections by default.
    /// </summary>
    [Fact]
    public void Test_PopulateEntity_Preserves_Collection_References_With_SystemTextJson()
    {
        var chain = CreateChainWithPopulate<BicycleWithAccessories>();
        ...
        // GOOD: The collection reference is PRESERVED with System.Text.Json Populate mode
        ReferenceEquals(targetBicycle.Accessories, originalCollectionRef)
            .Should()
            .BeTrue(
                "System.Text.Json with Populate preserves collection references - ORM tracking is safe!"
            );

        // NOTE: Like Newtonsoft.Json, System.Text.Json APPENDS to collections
